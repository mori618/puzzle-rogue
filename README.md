...
サイクルクリアごとに以下のロジックで商品が陳列されます。

1.  **通常枠 (4枠)**: 全スキル・パッシブの中からランダム。
    *   **セール品**: パッシブ「商談の極意」所持で、半額の商品が出現 (Lvに応じて2〜4個)。
    *   **高級品**: 購入回数(`Total Purchases`)が6回以上で、30%の確率でエンチャント付きトークンが出現 (価格+4)。
2.  **固定枠 1**: **ランダム強化** (5★) - 所持トークン1つのLvを上げる。
3.  **固定枠 2**: **ランダム付与** (条件:消費スター20以上) - ランダムなエンチャントを付与。
4.  **高級枠**: 購入回数が6回以上の場合、追加でエンチャント付与アイテムが出現。

- **リフレッシュ**: スターを消費して商品を新しく入れ替えます。使用するたびに費用が **1.5倍ずつ上昇** します。また、先のサイクルに進むにつれ、リフレッシュの**基礎費用も1.5倍ずつ増加**します（サイクル目標達成時、リフレッシュ費用は基礎費用にリセットされます）。
